  ...
  properties: {
    ...
    siteConfig: {
      appSettings: [
        ...
        {
          name: 'BlobStorage'
          value: 'DefaultEndpointsProtocol=https;AccountName=${outputStorageAccount.name};EndpointSuffix=${environment().suffixes.storage};AccountKey=${outputStorageAccount.listKeys().keys[0].value}'
        }
        {
          name: 'UpdateCron'
          value: '3 3 */2 * * *'
